...
int main(void)
{
  ...
  hwloc_cpuset_t set, set2, nocpunodeset;
  hwloc_obj_t node;
  ...
  nocpunodeset = hwloc_cpuset_alloc();
  ...
  if (hwloc_get_nbobjs_by_type(topology, HWLOC_OBJ_NODE)) {
    ...
    while ((node = hwloc_get_next_obj_by_type(topology, HWLOC_OBJ_NODE, node)) != NULL) {
      ...
      if (hwloc_cpuset_iszero(node->cpuset))
	hwloc_cpuset_set(nocpunodeset, node->os_index);
    }
  } else {
  ...
  hwloc_cpuset_foreach_begin(i, nocpunodeset) { numa_bitmask_setbit(bitmask, i); } hwloc_cpuset_foreach_end();
  ...
  hwloc_cpuset_foreach_begin(i, nocpunodeset) { nodemask_set(&nodemask, i); } hwloc_cpuset_foreach_end();
  assert(!memcmp(&nodemask, &numa_all_nodes, sizeof(nodemask_t)));

  hwloc_cpuset_free(set);
  hwloc_cpuset_free(nocpunodeset);
  ...
  if (node) {
    ...
    hwloc_cpuset_from_linux_libnuma_nodemask(topology, set, &nodemask);
    assert(hwloc_cpuset_isequal(set, node->cpuset));
    hwloc_cpuset_free(set);
