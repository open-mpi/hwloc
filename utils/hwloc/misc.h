				...
				int verbose, const char *callname)
{
  ...
  case HWLOC_UTILS_INPUT_FSROOT: {
    ...
    env = getenv("HWLOC_USE_X86");
    if (env)
      fprintf(stderr, "Cannot disable x86 because HWLOC_USE_X86 environment variable is already set to %s.\n", env);
    else
      putenv((char *) "HWLOC_USE_X86=none");
  ...
  case HWLOC_UTILS_INPUT_CPUID: {
    ...
    env = getenv("HWLOC_USE_X86");
    if (env)
      fprintf(stderr, "Cannot force x86-only mode because HWLOC_USE_X86 environment variable is already set to %s.\n", env);
    else
      putenv((char *) "HWLOC_USE_X86=only");
    /* normally-set flags are overriden by envvar-forced backends */
    if (flags & HWLOC_TOPOLOGY_FLAG_IS_THISSYSTEM)
      putenv((char *) "HWLOC_THISSYSTEM=1");
#else
...
hwloc_lstopo_show_summary_depth(FILE *output, size_t prefixmaxlen, hwloc_topology_t topology, int depth)
{
  ...
  if (nbobjs) {
    ...
    if (depth < 0) {
      ...
      hwloc_obj_type_snprintf(_types, sizeof(_types), hwloc_get_obj_by_depth(topology, depth, 0), HWLOC_OBJ_SNPRINTF_FLAG_LONG_NAMES, topology);
      types = _types;
    }
