...
static int hwloc_show_component_errors = 0;
#ifdef HWLOC_HAVE_PLUGINS
static int hwloc_plugins_verbose = 0;
static int hwloc_show_plugin_errors = 0;
...
hwloc_plugins_init(void)
{
  ...
  hwloc_show_plugin_errors = HWLOC_SHOW_ERRORS(HWLOC_SHOWMSG_PLUGINS) | hwloc_plugins_verbose;
...
hwloc_components_init(void)
{
  ...
  hwloc_show_component_errors = HWLOC_SHOW_ERRORS(HWLOC_SHOWMSG_COMPONENTS) | hwloc_components_verbose;
...
hwloc_disc_components_enable_others(struct hwloc_topology *topology)
{
  ...
  if (env && !strcmp(env, "x86,stop")) {
    ...
    if (HWLOC_SHOW_ERRORS(HWLOC_SHOWMSG_USER))
      fprintf(stderr, "hwloc: HWLOC_COMPONENTS=x86,stop deprecated, replacing with no_os,stop and HWLOC_USE_X86=only\n");
  ...
  if (env) {
	...
	curenv[s] = '\0';
        ...
        if (!strcmp(curenv+1, "x86")) {
          /* backward compat with x86 component in 2.x */
          if (HWLOC_SHOW_ERRORS(HWLOC_SHOWMSG_USER))
            fprintf(stderr, "hwloc: HWLOC_COMPONENTS=-x86 deprecated, replacing with HWLOC_USE_X86=none\n");
          topology->use_x86_mode = HWLOC_USE_X86_NONE;
        } else {
	...
	if (comp) {
	  ...
	  if (comp->phases & ~blacklisted_phases)
	    hwloc_disc_component_try_enable(topology, comp, 1 /* envvar forced */, blacklisted_phases);
          ...
          if (curenv == env) {
            if (HWLOC_SHOW_ERRORS(HWLOC_SHOWMSG_USER))
              fprintf(stderr, "hwloc: HWLOC_COMPONENTS starting with x86 is deprecated, replacing with HWLOC_USE_X86=first\n");
            topology->use_x86_mode = HWLOC_USE_X86_FIRST;
          } else {
            if (HWLOC_SHOW_ERRORS(HWLOC_SHOWMSG_USER|HWLOC_SHOWMSG_CRITICAL))
              fprintf(stderr, "hwloc: HWLOC_COMPONENTS with x86 is deprecated, falling back to default x86 configuration, use HWLOC_USE_X86 to change it.\n");
          }
        } else {
